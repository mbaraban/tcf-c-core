...
/* Linux agent has no CPU definition for ARMARCH7M, so force compile */
#if !defined(_WRS_KERNEL) && (CPU == ARMARCH7M) && !defined(ARMARCH7M)
#define ARMARCH7M -1
#endif
...
#   define REG_PC pc
#   define REG_PSR psr
#   define REG_CPSR cpsr
    { "r0",      REG_OFFSET(r[0]),      4, 0, 0},
    { "r1",      REG_OFFSET(r[1]),      4, 1, 1},
    { "r2",      REG_OFFSET(r[2]),      4, 2, 2},
    { "r3",      REG_OFFSET(r[3]),      4, 3, 3},
    { "r4",      REG_OFFSET(r[4]),      4, 4, 4},
    { "r5",      REG_OFFSET(r[5]),      4, 5, 5},
    { "r6",      REG_OFFSET(r[6]),      4, 6, 6},
    { "r7",      REG_OFFSET(r[7]),      4, 7, 7},
    { "r8",      REG_OFFSET(r[8]),      4, 8, 8},
    { "r9",      REG_OFFSET(r[9]),      4, 9, 9},
    { "r10",     REG_OFFSET(r[10]),     4, 10, 10},
    { "r11/fp",  REG_OFFSET(r[11]),     4, 11, 11},
    { "r12/ip",  REG_OFFSET(r[12]),     4, 12, 12},
    { "r13/sp",  REG_OFFSET(r[13]),     4, 13, 13},
    { "r14/lr",  REG_OFFSET(r[14]),     4, 14, 14},
    { "pc",      REG_OFFSET(pc),     4, 15, 15},
#if (CPU == ARMARCH7M)
    { "psr",     REG_OFFSET(psr),     4, 128, 128},
    { "basepri", REG_OFFSET(basepri),     4, -1, -1},
    { "demcr",   REG_OFFSET(demcr),     4, -1, -1},
#else	/* (CPU == ARMARCH7M) */
    { "cpsr",    REG_OFFSET(cpsr),     4, 128, 128},
    { "orig_r0", REG_OFFSET(ttbase),     4, -1, -1},
#endif	/* (CPU == ARMARCH7M) */
    { NULL,      0, 0,  0,  0},
};
#endif
...
#if (CPU == ARMARCH7M)
static RegisterDefinition * psr_def = NULL;
#else	/* (CPU == ARMARCH7M) */
...
#if (CPU == ARMARCH7M)
    if (read_reg(ctx, psr_def, psr_def->size, &cpsr) < 0) return -1;
#else	/* (CPU == ARMARCH7M) */
...
#if (CPU == ARMARCH7M)
        else if (r->offset == offsetof(REG_SET, REG_PSR)) {
            psr_def = r;
	}
#else	/* (CPU == ARMARCH7M) */
