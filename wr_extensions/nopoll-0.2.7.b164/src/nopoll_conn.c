...
int nopoll_conn_tls_receive (noPollConn * conn, char * buffer, int buffer_size)
{
	...
	while (tries < 50) {
		...
		/* next operation */
                __nopoll_conn_wait(conn->session, 1, 10000);
		tries++;
	}
...
int nopoll_conn_tls_send (noPollConn * conn, char * buffer, int buffer_size)
{
	...
	while (tries < 50) {
		...
		/* next operation */
                __nopoll_conn_wait(conn->session, 0, tries * 10000);
		tries++;
	}
	return res;
}
...
{
	char        buffer[4196];       /* max size of cookies is 4096 */
	int         buffer_size;
	noPollCtx * ctx = conn->ctx;
	...
	while (nopoll_true) {
		...
		buffer_size = nopoll_conn_readline (conn, buffer, sizeof(buffer));
