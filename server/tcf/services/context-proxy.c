...
#endif
#if ENABLE_PathMapHook
#include <tcf/main/pathmap_hook.h>
#endif
...
#if ENABLE_PathMapHook
    if (forward_pm) init_pmap_hook(p);
#endif
...
#if ENABLE_ContextMux && defined(context_get_memory_map)
#   undef context_get_memory_map

    static MemoryMap loc_target_map;
    static MemoryMap loc_client_map;
    ContextCache * cache = get_memory_map_cache(ctx);
    if (cache == NULL) {
        context_clear_memory_map(&loc_target_map);
        context_clear_memory_map(&loc_client_map);
    	if (context_get_memory_map(ctx, &loc_target_map) != 0)
    	    return -1;
        *client_map = &loc_client_map;
        *target_map = &loc_target_map;
    } else {
...
#   define context_get_memory_map sys_context_get_memory_map
	...
	*target_map = &cache->target_map;
    return 0;
#endif /* ENABLE_ContextMux && defined(context_get_memory_map) */
}
