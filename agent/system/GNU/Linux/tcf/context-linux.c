...
int context_write_mem(Context * ctx, ContextAddress address, void * buf, size_t size) {
    ...
    if (error == 0 && check_breakpoints_on_memory_write(ctx, address, buf, size) < 0) return -1;
    for (word_addr = address & ~((ContextAddress)word_size - 1); error == 0 && word_addr < address + size; word_addr += word_size) {
